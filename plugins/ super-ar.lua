...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
   local text = ' ⇣ تــ{✅}ــم 🌟تـفـعـیـل🌟الــبـــوت🚸⇣\n'..msg.to.title
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      local text = 'تــ{☑️}ــم تـعـطـیـل الــبــوت☂\n'..msg.to.title
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function unlock_group_ads(msg, data, target)
  ...
  if group_ads_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return ' ✅فـــتـــح 🔓 الــروابــــط هــــنــا🎈\n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    end
end
--------------------------------
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return '🌟تـمـ⚠️ فـتـحـ ❇️الـعـربـيـه فـي🌟 \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    end
end
------------------------------
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return '🌟الـاضـافـه بـالـتـاكـید 🌀تــم☑️ قـفـلـهـا فـي🌟 \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🌟تــ{☂‌}ــم قـفـل الـاضـافـه🌟 \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    end
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return '⭐️الـاضـافـه بـالـتـاكـيـد ⚡️تـمـ⚠️ فـتـحـهـا فـي⭐️ \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return '⭐️تـمـ⚠️ فـتـحـ ❇️الـاضـافـه فـي⭐️ \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    end
end
------------------------------
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return '⭐️تــ{☑️}ـم قــفــل الــجــمــاعــیــه الــاتــلــح⭐️ \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    else
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return '⭐️تـ{☑️}ـم قــفـل الـمـلـصـقـات لـاتـلـح⭐️ \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return '⭐️تـــم☑️قــفــل  الـمـلـصـقـات هـنـا☂⭐️ \n| '..msg.to.title..' |\n ('..(msg.from.id or 'لا يوجد')..')\n'
    end
end
